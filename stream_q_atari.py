...
class StreamQ(nn.Module):
    def __init__(self, n_actions=3, hidden_size=256, lr=1.0, epsilon_target=0.01, epsilon_start=1.0, exploration_fraction=0.1, total_steps=1_000_000, gamma=0.99, lamda=0.8, kappa_value=2.0, reset_for_random=False):
...
def main(env_name, seed, lr, gamma, lamda, total_steps, epsilon_target, epsilon_start, exploration_fraction, kappa_value, reset_for_random, debug, overshooting_info, render=False):
    ...
    agent = StreamQ(n_actions=env.action_space.n, lr=lr, gamma=gamma, lamda=lamda, epsilon_target=epsilon_target, epsilon_start=epsilon_start, exploration_fraction=exploration_fraction, total_steps=total_steps, reset_for_random=reset_for_random, kappa_value=kappa_value)
    if debug:
    ...
    for t in range(1, total_steps+1):
        ...
        agent.update_params(s, a, r, s_prime, terminated, random_action, overshooting_info)
        s = s_prime
